	...
	<table>
		...
		<tbody>
			...
			<c:forEach var="board" items="${bList }" varStatus="i">
				<tr>
					...
					<td><c:if test="${!empty board.boardFilename}"> O </c:if> <c:if
							test="${empty board.boardFilename}"> X </c:if></td>

					<td><fmt:formatNumber pattern="#,###,###" value="123000"></fmt:formatNumber>
					</td>
				</tr>
			</c:forEach>
		...
		<tfoot>
			...
			<tr>
				...
				<td>
					<button> <a href="/board/write.kh">글쓰기 </a></button>
				</td>
			</tr>
		</tfoot>
